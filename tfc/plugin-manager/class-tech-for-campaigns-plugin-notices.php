...
if ( ! class_exists( 'Tech_for_Campaigns_Plugin_Notices' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Tech_for_Campaigns_Plugin_Notices {
		...
		public function set_notices() {
			...
			// DEBUG:
			// delete_user_meta( get_current_user_id(), 'tfc-dismiss-plugin-notice_update' );
			// delete_user_meta( get_current_user_id(), 'tfc-dismiss-plugin-notice_install' );
			...
			if ( $to_install ) {
				...
				if ( get_user_meta( get_current_user_id(), 'tfc-dismiss-plugin-notice_install', true ) !== $check ) {
			...
			if ( $to_update ) {
				...
				if ( get_user_meta( get_current_user_id(), 'tfc-dismiss-plugin-notice_update', true ) !== $check ) {
		...
		public function display( $key, $value, $message ) {
			...
			?>
			<div id="tfc-plugin-<?php echo esc_attr( $key ); ?>-notice" class="<?php echo $class; ?>" data-notice-key="<?php echo esc_attr( $key ); ?>" data-notice-value="<?php echo esc_attr( $value ); ?>" data-security="<?php echo esc_attr( $nonce ); ?>" data-namespace="tfc" data-dismiss="<?php echo esc_attr( $this->args['nag_dismiss'] ); ?>">
		...
		public function dismiss() {
			...
			update_user_meta(
				get_current_user_id(),
				'tfc-dismiss-plugin-notice_' . $_POST['notice_key'],
				$_POST['notice_value']
			);
