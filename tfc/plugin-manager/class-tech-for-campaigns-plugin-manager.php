 ...
 * @package    Tech_for_Campaigns
 * @subpackage My_Plugin_Manager
 * @license    GPL-2.0+
 */

if ( ! class_exists( 'Tech_for_Campaigns_Plugin_Manager' ) ) {
	 ...
	 *
	 * 3. Includes the Tech_for_Campaigns_Plugins class and instantiates
	 * the object for holding suggested plugins.
	 *
	 * 4. Includes the Tech_for_Campaigns_Plugin_Notices class and
	 * instantiates the object to display notices to the
	 * user across all admin pages, EXCEPT the one we
	 * added by Tech_for_Campaigns_Plugin_Manager.
	 *
	 ...
	 * @since 1.0.0
	 */
	class Tech_for_Campaigns_Plugin_Manager {
		...
		private function set_plugins( $plugins ) {
			...
			/**
			 * Include Tech_for_Campaigns_Plugins class to store
			 * plugin data.
			 */
			require_once( $this->package['directory'] . '/class-tech-for-campaigns-plugins.php' );

			// Setup plugins.
			$this->plugins = new Tech_for_Campaigns_Plugins( $plugins, $this );
		...
		private function set_args( $args = array() ) {
			...
			$this->args = wp_parse_args( $args, array(
				'page_title'           => __( 'Suggested Plugins', 'tfc' ),
				// translators: 1: name of theme
				'views_title'          => sprintf( __( 'Suggested by %s', 'tfc' ), $this->package['name'] ),
				// translators: 1: name of theme
				'tab_title'            => sprintf( __( 'Suggested by %s', 'tfc' ), $this->package['name'] ),
				// translators: 1: name of theme
				'extended_title'       => sprintf( __( 'Suggested Plugins by %s', 'tfc' ), $this->package['name'] ),
				'menu_title'           => '', // Takes on page_title, when left blank.
				'parent_slug'          => 'themes.php',
				'menu_slug'            => 'suggested-plugins',
				'capability'           => 'install_plugins',
				'nag_action'           => __( 'Manage suggested plugins', 'tfc' ),
				'nag_dismiss'          => __( 'Dismiss this notice', 'tfc' ),
				// translators: 1: name of theme
				'nag_update'           => __( 'Not all of your active, suggested plugins are compatible with %s.', 'tfc' ),
				// translators: 1: name of theme, 2: number of suggested plugins
				'nag_install_single'   => __( '%1$s suggests installing %2$s plugin.', 'tfc' ),
				// translators: 1: name of theme, 2: number of suggested plugins
				'nag_install_multiple' => __( '%1$s suggests installing %2$s plugins.', 'tfc' ),
				'child_theme'          => false,
				'plugin_file'          => '',
			));
		...
		private function set_notices() {
			...
			/**
			 * Include Tech_for_Campaigns_Plugin_Notices class to setup
			 * and display admin-wide notices, if they're needed.
			 */
			require_once( $this->package['directory'] . '/class-tech-for-campaigns-plugin-notices.php' );
			...
			$notices = new Tech_for_Campaigns_Plugin_Notices( $args, $this, $this->plugins );
		...
		public function add_assets() {
			...
			wp_localize_script(
				...
				array(
					'thirdParty'   => __( 'Only plugins from wordpress.org can be installed directly here.', 'tfc' ),
					'notInstalled' => __( 'Plugin update skipped because it is not installed.', 'tfc' ),
				)
			);
		...
		public function display_page() {
			...
			<div id="suggested-plugins" class="wrap">
				...
				<?php if ( $plugins ) : ?>
					...
					<form method="post" id="bulk-action-form" data-namespace="tfc">
				...
				<?php else : ?>
					...
					<p><?php esc_html_e( 'No suggested plugins given.', 'tfc' ); ?></p>
		...
		private function display_table_nav( $which ) {
			...
			$actions = array(
				'install-selected'    => __( 'Install', 'tfc' ),
				'activate-selected'   => __( 'Activate', 'tfc' ),
				'deactivate-selected' => __( 'Deactivate', 'tfc' ),
				'update-selected'     => __( 'Update', 'tfc' ),
				'delete-selected'     => __( 'Delete', 'tfc' ),
			);
			...
			<div class="tablenav">
				<div class="actions bulkactions">
					...
					<label for="bulk-action-selector-<?php echo esc_attr( $which ); ?>" class="screen-reader-text">
						<?php esc_html_e( 'Select bulk action', 'tfc' ); ?>
					</label>

					<select name="action-<?php echo $which; ?>" id="bulk-action-selector-<?php echo esc_attr( $which ); ?>">

						<option value="-1"><?php esc_html_e( 'Bulk Actions', 'tfc' ); ?></option>
					...
					<?php
					submit_button( __( 'Apply', 'tfc' ), 'action', '', false,
						array(
							'id' => "do-action-$which",
						)
					);
			...
			<tr class="<?php echo esc_attr( $class ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-plugin="<?php echo esc_attr( $plugin['file'] ); ?>" data-status="<?php echo esc_attr( $plugin['status'] ); ?>" data-source="<?php echo esc_attr( $this->get_plugin_source( $plugin ) ); ?>">
				...
				<th scope="row" class="check-column">
					<label class="screen-reader-text" for="">
						...
						printf( __( 'Select %s', 'tfc' ), $plugin['name'] );
						?>
					</label>
				...
				<td class="column-installed-version">
					...
					<?php else : ?>
						<?php esc_html_e( 'Not Installed', 'tfc' ); ?>
					<?php endif; ?>
				</td>
		...
		private function display_actions( $plugin ) {
			...
			if ( $is_wp || 'not-installed' !== $plugin['status'] ) {
				...
				$actions['details'] = array(
					...
					'label'  => sprintf( __( 'More Information about %s', 'tfc' ), $plugin['name'] ),
					'target' => '_blank',
					'nonce'  => null,
				);
			...
			if ( 'not-installed' === $plugin['status'] ) {
				...
				if ( $is_wp ) {
					...
					$actions['install'] = array(
						...
						'text'   => __( 'Install', 'tfc' ),
						// translators: 1: name of plugin
						'label'  => sprintf( __( 'Install %s', 'tfc' ), $plugin['name'] ),
						'target' => '_self',
			...
			if ( 'inactive' === $plugin['status'] ) {
				...
				$actions['activate'] = array(
					'url'    => $url,
					'text'   => __( 'Activate', 'tfc' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Activate %s', 'tfc' ), $plugin['name'] ),
					'target' => '_self',
				);
				...
				$actions['deactivate'] = array(
					'url'    => $url,
					'text'   => __( 'Deactivate', 'tfc' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Deactivate %s', 'tfc' ), $plugin['name'] ),
					'target' => '_self',
			...
			if ( 'inactive' === $plugin['status'] ) {
				...
				$actions['delete'] = array(
					...
					'text'   => __( 'Delete', 'tfc' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Delete %s', 'tfc' ), $plugin['name'] ),
					'target' => '_self',
			...
			if ( $plugin['update'] ) {
				...
				$actions['update'] = array(
					...
					'label'  => sprintf(
						...
						__( 'Update %1$s to version %2$s', 'tfc' ),
						$plugin['name'],
						$plugin['new_version']
					),
					'target' => '_self',
				...
				$actions['update'] = array(
					'text' => __( 'Plugin is up-to-date', 'tfc' ),
				);
		...
		private function display_status( $plugin ) {
			...
			switch ( $plugin['status'] ) {
				case 'active':
					esc_html_e( 'Active', 'tfc' );
					break;

				case 'incompatible':
					esc_html_e( 'Incompatible', 'tfc' );
					break;

				case 'inactive':
					esc_html_e( 'Installed', 'tfc' );
					break;

				default:
					esc_html_e( 'Not Installed', 'tfc' );
			}
		...
		public function row_refresh() {
			 ...
			 * object is used.
			 */
			if ( empty( $_POST['namespace'] ) || 'tfc' !== $_POST['namespace'] ) {
				return;
			}
		...
		public function request() {
			...
			if ( isset( $_REQUEST['success'] ) ) {
				...
				if ( 'activate' === $_REQUEST['action'] ) {
					...
					$message = __( 'Plugin activated.', 'tfc' );
					...
					$message = __( 'Plugin deactivated.', 'tfc' );

				} elseif ( 'deactivate-selected' === $_REQUEST['action'] ) {

					$message = __( 'Plugins deactivated.', 'tfc' );
			...
			if ( $do_bulk && empty( $_REQUEST['checked'] ) ) {
				...
				$error = __( 'No plugins were selected.', 'tfc' );

			} elseif ( ! $do_bulk && empty( $_REQUEST['plugin'] ) ) {

				$error = __( 'No plugin slug was given.', 'tfc' );

			} elseif ( ! current_user_can( 'update_plugins' ) ) {

				$error = __( 'Sorry, you are not allowed to update plugins for this site.', 'tfc' );

			} elseif ( ! $do_bulk && empty( $plugins_data[ $_REQUEST['plugin'] ] ) ) {

				$error = sprintf(
					// translators: 1: slug of plugin being activated
					__( 'The plugin %s doesn\'t exist within the plugin manager\'s registered plugins.', 'tfc' ),
					$_REQUEST['plugin']
				);
			...
			if ( ! $error ) {
				...
				if ( $do_bulk ) {
					...
					if ( ! $plugins ) {
						$error = __( 'No valid plugins given for bulk action.', 'tfc' );
					}
				} else {
				...
				if ( 'activate' === $_REQUEST['action'] ) {
					...
					if ( $success ) { // At least one plugin needed to be successful.
						...
						$error = __( 'None of the selected plugins could be activated. Make sure they are installed.', 'tfc' );
